...
static void global_box( csBox box[static 1], csPoint vec )
{
   if ( eq_rect_cs( rect_cs( 0, 0, 0, 0 ), box->rect ) )
   {
      return;
   }
   box->rect.x += vec.x;
   ...
   {
      global_box( child, point_cs( box->rect.x, box->rect.y ) );
   }
}

void as_global_box_cs( csBox box[static 1] )
{
   global_box( box, point_cs( 0, 0 ) );
}

static bool draw_box( cVarRgb24Image image, csBox const box[static 1] )
{
   csPoint a = top_left_corner_cs( box->rect );
   csPoint b = bottom_right_corner_cs( box->rect );
   if ( not eq_rect_cs( rect_cs( 0, 0, 0, 0 ), box->rect ) )
   {
...
bool dump_box_layout_cs( cChars path,
                         ...
                         cErrorStack es[static 1] )
{
   cVarRgb24Image varImage = heap_var_rgb24_image_c_( box->rect.w, box->rect.h );
   
   bool res = false;
   if ( draw_box( varImage, box ) )
   {
      cRgb24Image image = image_copy_c_( varImage );
      res = write_p3_file_c( path, image, es );
   }
